...
def MyCrossAttnDownBlock2D_SD_v1_5_forward(self, x):
    ...
    help_scale = _clamp_help_scale(self.input_help_scale_factor)
    x = torch.cat([x, help_scale * x], dim=1)

    for i in range(2):
        x = RevBackProp.apply(x, self.rev_module_lists[i])
        x_split = x.chunk(2, dim=1)

        merge_scale = _clamp_merge_scale(self.merge_scale_factors[i])
        x_merge = x_split[0] + merge_scale * x_split[1]
        skip.append(x_merge)
...
def MyCrossAttnUpBlock2D_SD_v1_5_forward(self, x, skip):
    ...
    merge_scale = _clamp_merge_scale(self.merge_scale_factor)
    x = x_split[0] + merge_scale * x_split[1]
